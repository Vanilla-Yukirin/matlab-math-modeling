...
% 题注字体大小
\setbeamerfont{caption}{size=\scriptsize}      % 控制题注文字
\setbeamerfont{caption name}{size=\scriptsize} % 控制“图 x:”这部分
...
\begin{itemize}
  ...
  \item 文档齐全，内置帮助文档（按F1即可查看函数详细用法），相比去问AI会更加精确且全面
...
\begin{itemize}
  ...
  \item 免费软件，免费、开源、社区活跃，每个人都能自由的使用（Free Software）
\end{itemize}
\end{columns}
\end{frame}
...
\begin{frame}[fragile]{Matlab基本界面介绍}
  \begin{figure}
    \centering
    \includegraphics[width=0.55\textwidth]{Matlab_main_detail2.png}
    \caption{Matlab R2024b 界面示例}
  \end{figure}
\end{frame}
...
[matrix(1) matrix(2) matrix(3)] % 发现单参数访问顺序是 1 5 2 6 3 7
matrix(2,4) % 第2行第4列
...
\begin{frame}[fragile]{Matlab中的匿名函数}

\small
\textbf{匿名函数}是一行写完的小函数，不需要单独\texttt{.m} 文件，适合当作一些函数的\textbf{目标函数}进行传参。

\vspace{1mm}
\textbf{基本语法}
\begin{lstlisting}[style=matCN]
f = @(x) x.^2 + 3*x + 1;   % 定义 f(x)
f(2)                       % 调用：f(2)
\end{lstlisting}

\vspace{1mm}
\textbf{多变量匿名函数的两种常见写法}
...
h = @(v) (v(1)-2).^2 + (v(2)+1).^2;
h([3, 4])
best_v = fminsearch(h, [0, 0]);
\end{lstlisting}
\end{columns}

\vspace{1mm}
\small
匿名函数非常简单：“匿名函数名 = \texttt{@(...)} 函数表达式”，写起来非常短，稍后我们会频繁的用到它。
...
legend("sin^2(x)")       % 图例（说明线条含义）
  ...
  \scriptsize Tips: 画图模块的字符串大多都是支持上下标（\texttt{\textasciicircum}，\texttt{\_}）的，比如可以用\texttt{sin\textasciicircum 2(x)}来表示数学公式 $\sin^2(x)$。但这个特性有时候也会导致标签显示不正常。
...
\begin{frame}[fragile]{问题3}
  ...
  \begin{itemize}
    ...
    \item (4) 解方程 $f(x)=0.1$，求在 $x\in(0,2)$ 范围内的根（提示：使用 \texttt{vpasolve} 或 \texttt{fzero}）
    \item (5) 画图验证(4)的正确性
  \end{itemize}
...
\subsection{非线性规划模型与求解}


\begin{frame}[fragile]{什么是非线性规划\footnote{\tiny在上一节初级班中，王学长已经讲过如何使用 lingo 求解线性规划问题。我也觉得对于简单的线性规划问题而言，lingo 是一个更好的选择。所以这节课我就直接介绍非线性规划问题及其求解方法。}}
  \small 非线性规划问题 = 规划问题 + 至少有一个非线性的目标函数或约束。
  
  % \texttt{}
  
  \small 非线性规划问题在建模中大量出现（生产调度，最大利润/效率，路径规划……）
  ...
  非线性规划问题的特征：
\begin{itemize}\scriptsize
  \item 求某函数（在一定区域内）（满足特定条件下的）的极小值/极大值
\end{itemize}
\end{frame}


\begin{frame}[fragile]{例1：一维非线性函数}
  \small 求该函数的局部极小值：
$$
\min f(x) = x^3 - 6x^2 + 4x + 12,\quad x\in[-2,6]
$$

  \begin{figure}
  \centering
  \includegraphics[height=0.55\textheight]{fminbnd_plot.png}
  % \caption{函数 $f(x)$ 在 $[-2,6]$ 区间的图像}
  \end{figure}
\end{frame}


\begin{frame}[fragile]{例1：fminbnd 函数用法}
  \small fminbnd函数能够查找单变量函数在定区间上的最小值。
  \begin{lstlisting}[style=matCN]
[x_min, fval] = fminbnd(fun, a, b)
  \end{lstlisting}

  \small 参数说明：
  \begin{itemize}\scriptsize
    \item \texttt{fun}：单变量的目标函数
    \item \texttt{a, b}：搜索区间
    \item \texttt{x\_min}：极小点
    \item \texttt{fval}：对应函数值
  \end{itemize}

\end{frame}


\begin{frame}[fragile]{例1：fminbnd 求解代码}

  \codeinfo{Section4_5_fminbnd.m}
  \begin{lstlisting}[style=matCN]
f = @(x) x.^3 - 6*x.^2 + 4*x + 12; % 使用 .\textasciicircum 是为了支持向量输入
...
plot(x_min, fval, 'ro', 'MarkerSize',8, 'MarkerFaceColor','r'); % 在图上用红点标出极小值点
% 美化图像
grid on;
title("fminbnd (*@\CN{寻找一维极小值}@*)");
xlabel("x"); ylabel("f(x)");
...
\begin{frame}[fragile]{例1：结果可视化}
\begin{figure}
\centering
\includegraphics[height=0.75\textheight]{fminbnd_result.png}
% \caption{最优点标注于图中：$x^* = 3.6330$}
\end{figure}
\end{frame}


\begin{frame}[fragile]{例2：多变量无约束优化}
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.48\textwidth}
      \small 求该函数的极小值：
$$
f(x,y) = (x-2)^2 + (y+1)^2 + \sin(xy)
$$
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{figure}
        \centering
        \includegraphics[height=0.60\textheight]{fminsearch_surface.png}
        \caption{函数三维图像}
      \end{figure}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]{例2：fminsearch 函数用法\footnote{\href{https://ww2.mathworks.cn/help/matlab/ref/fminsearch.html}{fminsearch - Matlab帮助中心}}}
  非线性规划求解器 \texttt{fminsearch} 将使用无导数法搜索无约束的多变量函数的局部最小值。
...
[x_min, fval] = fminsearch(fun, x0)
  ...
  \small 参数说明：
\begin{itemize}\scriptsize
\item \texttt{fun}：以向量为输入的多变量函数
\item \texttt{x0}：初始点（重要）会从点x0处开始并搜索目标函数最小值
\item \texttt{x\_min}：局部最优解
\item \texttt{fval}：最优值
\end{itemize}
\end{frame}


\begin{frame}[fragile]{例2：fminsearch 求解代码}

  \codeinfo{Section4_6_fminsearch.m}
  \begin{lstlisting}[style=matCN]
clc; clear;
...
plot3(x_min(1), x_min(2), fval, 'r.', 'MarkerSize',30); % 在三维图上用红点标出极小值点
% 美化图像
title("fminsearch (*@\CN{寻找无约束极小值}@*)"); xlabel("x"); ylabel("y"); zlabel("f");
...
\begin{frame}[fragile]{例2：结果可视化}
\begin{figure}
\centering
\includegraphics[height=0.75\textheight]{fminsearch_result.png}
% \caption{最优点标注于三维图中}
\end{figure}
\end{frame}


\begin{frame}[fragile]{例3：带约束的非线性规划}
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.48\textwidth}
      \small 求该函数的极小值：
      \begin{align*}
      & \min_{x,y} && f(x,y) = x^2 + y^2 + 3\sin(x) \\
      & \text{s.t.} && x^2 + y^2 \le 4
      \end{align*}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{figure}
        \centering
        \includegraphics[height=0.60\textheight]{fmincon_contour.png}
        \caption{等高线 + 圆形可行域}
      \end{figure}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]{例3：fmincon 函数用法\footnote{\href{https://ww2.mathworks.cn/help/optim/ug/fmincon.html}{fmincon - Matlab帮助中心}}}
  \small 非线性规划求解器 \texttt{fmincon} 可以寻找约束非线性多变量函数的最小值，用于求解 \textbf{带约束的非线性优化问题}，目标形式为：

$$
\min_{x}\ f(x)
\quad\text{s.t.}\quad
\begin{cases}
c(x) \le 0, & \text{(非线性不等式约束)}\\[3pt]
ceq(x) = 0, & \text{(非线性等式约束)}\\[3pt]
A x \le b, & \text{(线性不等式约束)}\\[3pt]
A_{\mathrm{eq}} x = b_{\mathrm{eq}}, & \text{(线性等式约束)}\\[3pt]
\ell \le x \le u, & \text{(变量上下界)}
\end{cases}
$$

其中 $x$ 可以是标量、向量，也可以是矩阵（按列展开解释）。

\end{frame}


\begin{frame}[fragile]{例3：fmincon 函数用法}
  \small 使用格式如下：

\begin{lstlisting}[style=matCN]
[x_min, fval] = fmincon(fun, x0, A, b, Aeq, beq, lb, ub, nonlcon);
\end{lstlisting}

  其中：

  \begin{itemize}\scriptsize
    \item \texttt{fun}：目标函数句柄，返回标量 $f(x)$
    \item \texttt{x0}：初始点（非常重要，会影响最终结果）
    \item \texttt{A, b}：线性不等式 $Ax \le b$
    \item \texttt{Aeq, beq}：线性等式 $A_{\mathrm{eq}}x = b_{\mathrm{eq}}$
    \item \texttt{lb, ub}：变量下界与上界
    \item \texttt{nonlcon}：非线性约束函数，需返回 $c(x)$ 与 $ceq(x)$
  \end{itemize}

\end{frame}


\begin{frame}[fragile]{例3：fmincon 求解代码}

  \codeinfo{Section4_7_fmincon.m}
  \begin{lstlisting}[style=matCN]
clc;clear;
...
title("fmincon (*@\CN{求解带圆形约束的非线性优化问题}@*)");
xlabel("x");ylabel("y");
fprintf("best[x,y]=[%.4f,%.4f],f=%.4f\n",x_min(1),x_min(2),fval);
  \end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{例3：结果可视化}
\begin{figure}
\centering
\includegraphics[height=0.75\textheight]{fmincon_result.png}
% \caption{最优点位于可行域内}
\end{figure}
\end{frame}
...
方法 & 变量维度 & 是否可约束 & 适用场景 \\
\hline
\texttt{fminbnd} & 一维 & 有区间 $[a,b]$ & 单变量极小值 \\
\hline
\texttt{fminsearch} & 多维 & 无约束 & 多变量光滑函数局部极小 \\
\hline
\texttt{fmincon} & 多维 & 支持各种约束 & 真正的非线性规划问题 \\
\hline
\end{tabular}
\caption{三种优化方法的区别与使用场景}
\end{table}
\end{frame}
...
\begin{frame}[fragile]{问题1描述}
  % textblock* 参数: {width}(x_pos, y_pos)
  % x_pos 和 y_pos 的单位默认是 cm。
  % 0,0 是页面的左上角。你需要根据实际效果调整 x_pos 和 y_pos 来放置图片。
  \begin{textblock*}{0.38\textwidth}(0.70\textwidth, 0.45\textheight) % 假设图片从页面60%宽度处开始，距离顶部15%高度
    \centering
    \includegraphics[height=0.30\textheight]{Web_logo.png}
    % 在textblock里也可以放caption，但由于是绝对定位，caption会紧随图片
  \end{textblock*}
  ...
  \begin{lstlisting}[style=matCN]
timestamp,year,month,day,hour,minute,second,number
1762203562,2025,11,4,4,59,22,3825
1762204847,2025,11,4,5,20,47,3826
1762206048,2025,11,4,5,40,48,3827
...
1763019325,2025,11,13,15,35,25,4596
  \end{lstlisting}
...
\begin{frame}[fragile]{问题1-1,1-2}
  ...
  \begin{itemize}\scriptsize
    \item 将时间字段转换为“从 2025-11-04 00:00:00 起经过的总秒数” $t$
  ...
  \begin{itemize}\scriptsize
    \item 构造两个自变量：
      \begin{itemize}\scriptsize
        ...
        \item \texttt{minute\_of\_day}：一天中的第几分钟（$hour\times60 + minute$）
      \end{itemize}

    \item 构造模型：
    $$number = \beta_0 + \beta_1 day + \beta_2 minute + \varepsilon$$

    \item 双变量回归模型的效果相比单变量模型是否提升
...
\subsection{服务器机房的能耗优化}

\begin{frame}[fragile]{问题2描述}

  \scriptsize 冬天到了，作为 HatsuNova 项目组成员，你需要对机房服务器的散热系统进行参数调整。
  
  \scriptsize 既要保证散热效果，又要尽可能降低电力成本。
  
  \begin{figure}
    \centering
    \includegraphics[height=0.25\textheight]{HatsuNova.png}
  \end{figure}
\end{frame}


\begin{frame}[fragile]{问题2-1}

  \small \textbf{任务 1：单变量优化}
  
假设服务器仅有一个主散热风扇。风扇转速 $x$（单位：千转/分）与系统的总能耗成本 $f(x)$ 满足以下经验公式：

$$ f(x) = x^3 - 9x^2 + 24x + 30 $$

其中，风扇转速的安全调节范围是 $x \in [0, 6]$。

\begin{itemize}\scriptsize
    \item 请分别尝试使用 \texttt{fminbnd}、\texttt{fminsearch} 和 \texttt{fmincon} 寻找使能耗最低的转速 $x$。
    \item 思考它们的使用场景有何不同
\end{itemize}
...
\begin{frame}[fragile]{问题2-2}
  \small 为了应对高负载，机房升级了“风冷+水冷”混合系统。
  
  \small HatsuNova 的项目组成员遇到了新的难题：如何平衡噪音、震动、功耗和散热能力？

  \small \textbf{任务 2：多变量非线性约束规划}

  \small 变量 $x_1$：风冷风扇转速；\quad 变量 $x_2$：水冷水泵流速。
  
  \small 目标函数（总噪音与震动最小化）：

$$ \min f(x_1, x_2) = (x_1 - 2)^2 + (x_2 - 2.5)^2 $$
...
\begin{enumerate}\scriptsize
    \item 功耗限制：风冷和水冷的总功耗不能超过 12.5 单位：
    $$ 2x_1 + 3x_2 \le 12.5 $$
    \item 散热能力：为了刚好压制 CPU 温度，两者的散热效率乘积必须严格等于 6：
    $$ x_1 \cdot x_2 = 6 $$
    \item 边界条件：$0 \le x_1 \le 5, \quad 0 \le x_2 \le 5$。
\end{enumerate}
...
\begin{frame}[fragile]{答疑\footnote{\href{https://lug.ustc.edu.cn/wiki/doc/howtoask/}{提问指南}}\footnote{\href{https://lug.ustc.edu.cn/wiki/doc/smart-questions/}{提问的智慧}}}

  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.48\textwidth}
今天晚上和周六周日，我会在数模5群里答疑。大家可以把不懂的地方发到群里，我和数模协会的同学会尽量帮大家解答。

    \end{column}
    \begin{column}{0.48\textwidth}

\small 希望大家在提问时，能够使问题易于回复

\scriptsize 1. 能够使用清晰、正确、精准且合乎语法的语句描述问题和操作流程。

\scriptsize 2. 使用易于读取且标准的格式发送问题（完整的代码/代码截图，不要拍屏，必要时可以发文件）。

\scriptsize 3. 精确地描述问题并言之有物。

      \begin{figure}
        \centering
        \includegraphics[height=0.25\textheight]{problem_price.jpg}
        \caption{开个小玩笑}
      \end{figure}
    \end{column}
  \end{columns}
