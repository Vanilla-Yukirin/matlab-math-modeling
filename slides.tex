...
\lstdefinestyle{matCN}{
  ...
  morecomment=[l]\%,  % Matlab 单行注释（可保留也可省略）
}
...
\begin{frame}[fragile]{向量是简单的矩阵}
  向量是特殊的矩阵，向量可以看作是只有一行或者一列的矩阵。

  \texttt{\scriptsize同理，标量可以看作是1行1列的矩阵。这也是为什么Matlab中万物皆矩阵}
