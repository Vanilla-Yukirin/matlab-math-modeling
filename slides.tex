...
legend("sin(x)^2")       % 图例（说明线条含义）
  ...
  \scriptsize Tips: 画图模块的字符串大多都是支持上下标（\texttt{\textasciicircum}，\texttt{\_}）的，比如可以用\texttt{sin(x)\textasciicircum 2}来表示数学公式 $\sin(x)^2$。但这个特性有时候也会导致标签显示不正常。
...
$$
  ...
  \begin{figure}
  \centering
  \includegraphics[height=0.40\textheight]{fminbnd_plot.png}
...
\begin{figure}
\centering
\includegraphics[height=0.65\textheight]{fminbnd_result.png}
...
\begin{frame}[fragile]{例2：fminsearch 函数用法\footnote{\href{https://ww2.mathworks.cn/help/matlab/ref/fminsearch.html}{fminsearch - Matlab帮助中心}}}
  非线性规划求解器 \texttt{fminsearch} 将使用使用无导数法搜索无约束的多变量函数的局部最小值。
...
\begin{figure}
\centering
\includegraphics[height=0.65\textheight]{fminsearch_result.png}
\caption{最优点标注于三维图中}
\end{figure}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[height=0.65\textheight]{fmincon_result.png}
\caption{最优点位于可行域内}
\end{figure}
\end{frame}
